...
<ComponentsDisplay>
  <div className="flex flex-col items-center gap-2">
    <div className="grid grid-cols-8 border-stroke bg-gray-600 border rounded-lg overflow-hidden text-gray-100 [&>*:nth-child(n+53):not(:nth-child(n+57):nth-child(-n+60))]:bg-blue-800 [&>*:nth-child(n+53):not(:nth-child(n+57):nth-child(-n+60))]:text-blue-200">
      {new Array(64).fill(null).map((_, i) => (
        <div
          key={i}
          className="text-xs font-mono w-8 h-8 flex items-center justify-center"
        >
          {252 - i * 4}
        </div>
      ))}
    </div>
    <div className="text-sm text-gray-200">Numbers represent LSb positions</div>
  </div>
</ComponentsDisplay>
